...
当时我们虽然不清楚，但是我们无意中遵从了Brooks的建议[BR86]，他坚持软件系统应该渐进地发展，而不应按照传统的方式来设计，这种快速原型的方法应该成为设计方法论。

我们本来没打算开发新语言，只是想尝试找到一种优雅的方式为一个简单的电信应用编写一个PABX（Private Automatic Branch Exchange）,Erlang不过是过程中的副产品。［DA86］记录了一些基础性的工作，[AR86]记录了一些最初的语言实验的结论。

在撰写本论文的过程中，我们还遵从了Parnas和Clements的建议[PA86]，也就是，we present the development of our language as if it had been the result of a rational design process - whereas in fact the real design process is virtually impossible to describe.

开发取得的成功在很大程度上取决于Prolog（确实，如果不是Prolog，很难说像Erlang这样的语言能不能出现）。Prolog提供的交互环境让我们在开发过程中可以快速地变化。从更细节的层面上讲，Prolog的实现给我们提供了关键的灵感，让我们创造出一个有效版本的Erlang。Prolog中的**错误**（我们在Erlang中已经避免了）警示了我们不要做什么！

Erlang支持一些Prolog中不包括的机制。Erlang是并发的、实时的、分布式的函数式语言，适合构建大型的嵌入式实时系统。它包含很多模块，可以创建大规模的系统，在运行时系统中，代码可以随时更改，并且有一套成熟的错误捕获机制，这些都保证了Erlang可以构建高度健壮的软件系统。更多的细节可以在这个找到[AR89，AR90]。

开始事，Erlang只是一个简单的Prolog原解释器，然后添加了并发和各种错误处理机制的概念。最终，Erlang成为了一门独立的语言。在第二部分中，将详细描述Erlang解释器背后的理念。

当构造了一个简单的Erlang解释器后，我们就迫不及待地用它解决真实的问题了。有一组程序员使用它对真实的电信应用进行建模。其中一些成果记录在[PE91]中了。

第三部分描述了用户组对于语言开发的影响。

随着语言的发展，它获得了自己的语法和它与Prolog缺陷之间的联系——第四部分讲述了语言的开发。

迄今为止，开发过程是极度不正规的——解释器（几乎每天在变）是语言唯一的定义。当解释器成熟时，并且变化不再频繁后，适当的文档就是必须的了。这个变化将在第五部分讲述。

大概三年后，用户组的这门语言感到满意了。其中一项研究编写了20000行Erlang代码实现了电信交换系统中的各种特性。这些特性曾经用传统的语言实现过。使用Erlang后，可以减少7到22倍的工作量。现在，语言的性能成为了主要的关注点。尽管大家对于语言强大的表现力非常满意，但是他们还需要一个比现有解释器至少快40倍实现。

第六部分讲述了我们让语言更快的一些尝试。The first attempt was to cross compile Erlang to Prolog or STRAND.

对于下一阶段开发的灵感来自于WAM[WA83]——由于Erlang源自与Prolog，所以试Prolog变快的技术理所应当地适用于Erlang。我们将讲述如何实现一个更快的Erlang。

第七节描述了语言当前的状态。

### 2. 第一版解释器 ###

首先，让我们讨论一个简单的Prolog解释器，它能够运行Erlang非常有限的子集。

第一个解释器是个简单的Prolog元解释器，并在Prolog的基础上增添了可中断进程的概念。这个解释器只是创造一个适合开发健壮的、并发的实时系统的起点，接下来还进行了一系列的试验，来检验这个语言应该包含哪些特性。

第一个解释器被快速地修改（和重写），来包含在一系列的详细研究中确定下来的功能。这些研究的目的是为了发现电信领域程序设计语言的应有的特性[DA86]。

在那段时期里，解释器可以在几天里全部被重写，几个小时内就能做个简单的扩展。
在下面的几段中，我们描述了第一个解释器用到的几项基本技术。

## 2.1 中断计算 ##

第一步实现**可中断的**计算。我们希望编写一个元解释器，可以在任何时间点中断，稍后也可以重新**恢复**计算。
...
上面的代码会归并形如[Item1, Item2, ….]的列表。这里假设由花括号“｛”和“｝”括起来的部分是Prolog内建的目标，其他的则是用户自定义的规则。

作为例子，我们来看一下广为人知的reverse和factorial函数如何用eqn/2的风格实现：
{% codeblock lang:erlang %}
...
过程中保留在列表中的目标可以计算的任何点上被归并，因为这个列表只是个普通的Prolog数据结构，用户可以按照任何必要的方式操作它们。这里我们需要的是在完成固定数量的归并步骤后，能够中断计算。
...
前一节的归并机制可以作为基础，实现一个简单的“round robin”调度器（multi_reduce/1）。它以形如job(N, Goals)的任务（Task）列表作为参数。  调度器允许每个任务最多执行20步归并，然后进入下一个任务，只到所有任务全部完成。N是任务的名字，Goals是任务中包含的目标列表。

**从ps文件中copy**

我们可以加入一些write语句，这样方便观察程序的行为：

**从ps文件中copy**

从输出我们能够看出三个目标nrev([a,b,c,d,e,f,g,h])，nrev([1,2,3,4,5], R1)和fact(10, Fact)在并发执行。
...
注意在这个本版的解释器支持的元语言中，write/1和nl/0已经成为原语函数了，可以直接写它们来调用，不再需要花括号括起来了。添加其他的原语也非常容易，就像write/1和nl/0所做的一样。
...
这是最后一个最复杂的示例。我们将综合前面两节的想法，编写一个reduce2/4解释器，它支持函数式编程风格、创建进程并在进程间传输简单的消息。

每个进程都是一个数据结构：job(Id, Goals, MailBox)，Id是进程名，Goals计算中包含的未解决的目标列表，MailBox是一个消息的列表，里面包含由其他进程发送的，当还没有被该进程接收的消息。

目标reduce2(Goals, Id, MailBox, Jobs)代表了一个简单的多任务操作系统的当前状态。Goals，Id和MailBox表现了当前的执行进程，Jobs是所有被中止的进程的列表。我们将这些进程成为“环境”。

**从ps文件中copy**

元语言的原语spawn(Id, Goals)会调度一个新的进程，名字是Id，目标列表是Goals，send（Id， Message）会将消息Message发送到名为Id的进程。receive函数可以返回进程邮箱中的第一个值。

这个简单的解释器所用的调度算法也非常直接，一个进程不再执行的条件只有两个：要么它终止了，要么它试图接收（receive）消息时，邮箱中却是空的。

在实际情况下，我们会使用更加复杂的调度算法，它将包含时间片，以及当进程不能再继续时，上下文切换。

**从ps文件中copy**

相应的eqn2/2定义了一个新的元语言——如下是用这个元语言写的简单的程序：
...
另外我们完全没有考虑性能的问题——真实的解释器会使用difference list来消除程序中的append，这样性能或多或少会有些改善（但是可读性就打折扣了）。

早期阶段的语言特性可以在数小时内实现——讨论语言特性的时间永远比实现特性花的时间长。在语言解释器的开发阶段，它经历了无数次的细小变化，以及一些重大的修改，通常需要完全重写。

解释器本身被证明是健壮、高度可移植的。第一版是在VAX 11/750上使用NU Prolog实现的，紧接着被切换到同一台机器上的Quintus Prolog。后来用户组的成员们转到PC上，所以又出现了PC/AT上的ALS Prolog版本。我们又把它迁移到SICStus Prolog，运行环境也从VAX转到了SUN3/60和SPARC。
...
实现这些特性大约使用1100行Prolog代码。

第一个解释器用来实验各种不同的语言构件、调度机制和编程风格等等，目标是找到一种方便、优雅的方式来解决简单的电信编程问题。
...
用户组的形成过程促进了开发过程的稳定性。CSL成员XXXXXXXXXXXXXXXX

在历时一年的时间里，CSL和用户组成员每周见一到两次面，以沟通彼此之间的问题——这个过程的价值是难以言表的，使用者拥有无与伦比的应用开发经验，因此建议了很多有价值的语言特性（而且把它们添加到解释器中非常简单）。CSL成员会说服应用开发者，告诉他们为什么有些特定的命令式编程风格会产生问题，从而应该避免。

这个过程虽然有时候会令人沮丧，但是后来回顾的时候，发现这是一个关键的因素，得以让这个语言后来拥有足够的接受度。

有一点非常有趣，一旦用户组开始使用语言，语言就又开始变化了——XXXXXXXXXXXXXXXXXX
...
在这个语言被使用了一年后，达到了相对稳定的状态，于是我们写了Erlang的解析器，为语言自身赋予了语法，这样我们也可以创造在Prolog中不合法的新语法和结构。

上面的代码用新语法可以写成这样：

**从ps文件中copy**
 
有了新语法后没多久，我们就采纳了纯函数式的风格，避免了不必要的临时变量，上面的代码可以这样写：
...
我们编写了Erlang到我们的机器（JAM）的编译器以及JAM的模拟器。最初，编译器和模拟器都是用Prolog实现的。结果它们慢极了！(5 Erps (Erlang Reductions Per Second), 0.5 Erps when tracing the instructions)——xxxxxxxxxxxxxx
