---
layout: post
title: "基于Prolog构造Scheme解析器之一：读取源文件"
...
选择Scheme的原因在于它足够简单而且有趣。另外，通过实现这个简单的运行时环境，我们也可以领略Prolog的独特之处。Prolog最为应用最广泛的逻辑式程序设计语言，在描述文法生成式时具有命令式语言无可比拟的优势；另外最为声明式语言，Prolog不涉及运算的细节，因此程序长度也比命令式语言缩短了不少。
...
为了实现这个谓词，我们还需要完成一些工作，比如如何获取Prolog运行时内部的文件句柄？如果从文件句柄中依次读取数据？如何知道是否读到了文件的尾部？
